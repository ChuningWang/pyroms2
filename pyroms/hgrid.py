...
def rho_to_vert_geo(lonr: np.ndarray, latr: np.ndarray,
    ...
    # North edge
    x[Mm, 0:-2] = xr[Mm-1, 0:-1] - (xp[Mm-2, :] - xr[Mm-1, 0:-1])
    x[Mm, -2:] = xr[Mm-1, -2:] - (xp[Mm-2, -2:] - xr[Mm-1, -2:])
    y[Mm, 0:-2] = yr[Mm-1, 0:-1] - (yp[Mm-2, :] - yr[Mm-1, 0:-1])
    y[Mm, -2:] = yr[Mm-1, -2:] - (yp[Mm-2, -2:] - yr[Mm-1, -2:])

    # South edge
    x[0, 0:-2] = xr[0, 0:-1] - (xp[0, :] - xr[0, 0:-1])
    x[0, -2:] = xr[0, -2:] - (xp[0, -2:] - xr[0, -2:])
    y[0, 0:-2] = yr[0, 0:-1] - (yp[0, :] - yr[0, 0:-1])
    y[0, -2:] = yr[0, -2:] - (yp[0, -2:] - yr[0, -2:])

    # East edge
    x[0:-2, Lm] = xr[0:-1, Lm-1] - (xp[:, Lm-2] - xr[0:-1, Lm-1])
    x[-2:, Lm] = xr[-2:, Lm-1] - (xp[-2:, Lm-2] - xr[-2:, Lm-1])
    y[0:-2, Lm] = yr[0:-1, Lm-1] - (yp[:, Lm-2] - yr[0:-1, Lm-1])
    y[-2:, Lm] = yr[-2:, Lm-1] - (yp[-2:, Lm-2] - yr[-2:, Lm-1])

    # West edge
    x[0:-2, 0] = xr[0:-1, 0] - (xp[:, 0] - xr[0:-1, 0])
    x[-2:, 0] = xr[-2:, 0] - (xp[-2:, 0] - xr[-2:, 0])
    y[0:-2, 0] = yr[0:-1, 0] - (yp[:, 0] - yr[0:-1, 0])
    y[-2:, 0] = yr[-2:, 0] - (yp[-2:, 0] - yr[-2:, 0])
